...
public class FoldingAtHomeStatsCommand extends BotCommand implements Describable {
	...
	protected void processSlash(SlashCommandInteractionEvent event) {
		try {
			event.replyEmbeds(FoldingEmbed().build()).queue();
		} catch (IOException e) {
			event.reply("There was an issue reaching the Folding@Home API. please try again later.")
					.setEphemeral(true).queue();
		} catch (JSONException e){
			event.reply("Folding@Home has changed their API structure (again) and Ranzer is unaware. " +
